...
const postMatchesCategory = (post, categorySlug) =>
  post?.categorySlug === categorySlug || post?.brandCategorySlug === categorySlug;
...
const ensureCategorySummary = (categorySlug) => {
  ...
  const categoryPosts = posts.filter((post) =>
    postMatchesCategory(post, categorySlug),
  );
...
export async function getPostByCategoryAndSlug(categorySlug, slug) {
  const post = posts.find(
    (item) => postMatchesCategory(item, categorySlug) && item.slug === slug,
  );
...
export async function getPostsByCategory(categorySlug) {
  const categoryPosts = posts
    .filter((item) => postMatchesCategory(item, categorySlug))
    .sort((a, b) => {
...
export async function getHomePlacements() {
  ...
  const showcaseHighlight = resolvePosts(showcaseConfig.highlight)[0] ?? null;
  ...
  const latestPosts = resolvePosts(home.latestPosts);
  ...
  return {
    ...
    // Latest posts grid when configured; empty means use default ordering on the page.
    latestPosts,
  };
}
...
// Get sidebar featured posts for all pages
export async function getSidebarFeaturedPosts() {
  const sidebarConfig = placements.sidebar ?? {};
  const featuredPostKeys = sidebarConfig.featuredPosts ?? [];
  const featuredPosts = resolvePosts(featuredPostKeys);
